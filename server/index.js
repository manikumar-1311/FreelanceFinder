...
io.on("connection", (socket) =>{
    const now = Date.now();
    const timestamps = connectionTimestamps.get(socket.handshake.address) || [];
    // Filter timestamps older than 1 minute
    const recentTimestamps = timestamps.filter(ts => now - ts < 60000);
    recentTimestamps.push(now);
    connectionTimestamps.set(socket.handshake.address, recentTimestamps);

    if(recentTimestamps.length > 20){
        console.warn(`High connection rate from IP ${socket.handshake.address}`);
...
mongoose.connect(process.env.MONGODB_URI,{
    ...
    // Add API endpoint to check DB connection status
    app.get('/db-status', (req, res) => {
        ...
        res.json({ status: states[db.readyState] || 'unknown' });
    });
